...
export function useGraphDialogs() {
  ...
  const [rebaseTarget, setRebaseTarget] = useState<{ onto: string; interactive?: boolean } | null>(
    null
  );
  ...
  const [patchTarget, setPatchTarget] = useState<{ hashes: string[]; subjects: string[] } | null>(
    null
  );
