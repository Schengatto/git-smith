...
export const RebaseDialog: React.FC<Props> = ({
  ...
  const _handleContinue = async () => {
    ...
    setError(null);
    try {
      await runGitOperation("Rebase --continue", () => window.electronAPI.branch.rebaseContinue());
      await Promise.all([refreshInfo(), refreshStatus(), loadGraph()]);
  ...
  const handleAbort = async () => {
    ...
    setError(null);
    try {
      await runGitOperation("Rebase --abort", () => window.electronAPI.branch.rebaseAbort());
      setRebaseInProgress(false);
      await Promise.all([refreshInfo(), refreshStatus(), loadGraph()]);
      onClose();
    } catch (err: unknown) {
  ...
  const handleSkip = async () => {
    ...
    setError(null);
    try {
      await runGitOperation("Rebase --skip", () => window.electronAPI.branch.rebaseSkip());
      await Promise.all([refreshInfo(), refreshStatus(), loadGraph()]);
      const stillInProgress = await window.electronAPI.branch.isRebaseInProgress();
      if (!stillInProgress) {
        setRebaseInProgress(false);
            ...
            >
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
                <label style={checkboxLabelStyle}>
                  <input
                    type="checkbox"
                    checked={committerDateIsAuthorDate}
          ...
          {loadingCommits ? (
            ...
            entries.map((entry, i) => (
              ...
              >
                {interactive && (
                  <select
                    ...
                    style={{
                      ...
                      color:
                        ACTIONS.find((a) => a.value === entry.action)?.color ||
                        "var(--text-primary)",
                      fontSize: 10,
                      fontWeight: 600,
                      outline: "none",
                ...
                <span
                  className="mono"
                  style={{ width: 90, textAlign: "right", color: "var(--text-muted)" }}
                >
