...
describe("PatchCreateDialog", () => {
  ...
  it("renders Create Patch confirm button and Cancel button", () => {
    render(
      <PatchCreateDialog open={true} onClose={vi.fn()} hashes={["abc1234"]} subjects={["fix: x"]} />
    );
  ...
  it("calls onClose when Cancel is clicked", () => {
    const onClose = vi.fn();
    render(
      <PatchCreateDialog open={true} onClose={onClose} hashes={["abc1234"]} subjects={["fix: x"]} />
    );
  ...
  it("opens directory browser and calls patch.create on Create Patch click", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(mockElectronAPI.patch.create).toHaveBeenCalledWith(["abc1234"], "/tmp/patches");
    });
  });
...
describe("PatchApplyDialog", () => {
  ...
  it("renders patch path input as readonly", () => {
    render(<PatchApplyDialog open={true} onClose={vi.fn()} />);
    const input = screen.getByPlaceholderText("Select a patch file...") as HTMLInputElement;
