...
describe("ChangelogDialog", () => {
  ...
  it("calls generate when tag is selected", async () => {
    ...
    render(<ChangelogDialog open={true} onClose={vi.fn()} commitHash="abc1234" />);
    await waitFor(() => {
      expect(mockElectronAPI.changelog.generate).toHaveBeenCalledWith(
        "v1.0.0",
        "abc1234"
      );
    });
  ...
  it("shows custom ref input when Custom ref is selected", async () => {
    ...
    expect(
      screen.getByPlaceholderText("branch, tag, or commit hash")
    ).toBeInTheDocument();
  });
  ...
  it("renders in window mode without overlay wrapper", () => {
    render(
      <ChangelogDialog open={true} onClose={vi.fn()} commitHash="abc1234" mode="window" />
    );
    expect(screen.getByText("Close")).toBeInTheDocument();
  });
  ...
  it("resets state when dialog re-opens", async () => {
    ...
    await waitFor(() =>
      expect(mockElectronAPI.changelog.getTagsBefore).toHaveBeenCalledOnce()
    );
    ...
    await waitFor(() =>
      expect(mockElectronAPI.changelog.getTagsBefore).toHaveBeenCalledTimes(2)
    );
  });
});
