    ...
    {TABS.map((t) => (
      <button
        ...
        style={{
          ...
          border: "none",
          borderBottom:
            value === t.value ? "2px solid var(--accent)" : "2px solid transparent",
...
const ContributorsTable: React.FC<{ entries: ContributorTimelineEntry[] }> = ({
  entries,
}) => {
              ...
              {allDates.map((date) => (
                <td
                  ...
                  style={{
                    ...
                    textAlign: "right",
                    color:
                      (row.byPeriod[date] ?? 0) > 0 ? "var(--blue)" : "var(--text-muted)",
                    borderBottom: "1px solid var(--border-subtle)",
                  }}
                >
                  {row.byPeriod[date] ?? "—"}
  ...
  const fetchData = useCallback(async (currentTab: Tab, currentPeriod: Period) => {
    ...
    try {
      if (currentTab === "timeline") {
        ...
        const data =
          await window.electronAPI.statsAdvanced.contributorsTimeline(currentPeriod);
  ...
  return (
    <ModalDialog open={open} title="Advanced Statistics" onClose={onClose} width={750}>
      ...
      {((tab === "timeline" && timelineData && timelineData.length > MAX_BARS) ||
        ...
        (tab === "contributors" &&
          contributorsData &&
          Array.from(new Set(contributorsData.map((e) => e.date))).length >
            MAX_BARS)) && (
        <div
          style={{
            marginTop: 8,
