...
describe("CheckoutDialog", () => {
  ...
  it("shows warning when Reset strategy is selected", () => {
    ...
    expect(
      screen.getByText(/all uncommitted changes will be discarded/)
    ).toBeInTheDocument();
  });

  it("shows direct branch name in direct mode", () => {
    render(
      <CheckoutDialog open={true} onClose={vi.fn()} branchName="feature/my-branch" />
    );
  ...
  it("shows detached HEAD option when refs include local branches", () => {
    ...
    render(
      <CheckoutDialog open={true} onClose={vi.fn()} refs={refs} commitHash="abc123" />
    );
