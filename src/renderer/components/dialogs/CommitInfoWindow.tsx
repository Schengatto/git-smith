            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              >
                ...
                >
                  ...
                  {totalAdditions > 0 && (
                    <span style={{ color: "var(--green)", marginLeft: 8 }}>+{totalAdditions}</span>
                  )}
                  {totalDeletions > 0 && (
                    <span style={{ color: "var(--red)", marginLeft: 4 }}>-{totalDeletions}</span>
                  )}
                </div>
              </div>
...
const HashLink: React.FC<{ hash: string; onClick?: (hash: string) => void }> = ({
  hash,
  onClick,
}) => (
  <span
...
const BadgeList: React.FC<{ items: string[]; emptyText: string; color: string }> = ({
  items,
  emptyText,
  color,
}) => {
