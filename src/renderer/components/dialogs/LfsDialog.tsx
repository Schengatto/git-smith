        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 11, color: "var(--text-muted)" }}>
              ...
              {info?.endpoint && <span title={info.endpoint}>Endpoint: {info.endpoint.slice(0, 40)}...</span>}
            </div>
            ...
            <div style={{ display: "flex", gap: 0, borderBottom: "1px solid var(--border-subtle)" }}>
              {(["overview", "files"] as const).map((t) => (
                ...
                >
                  {t === "overview" ? `Tracked Patterns (${status.tracked.length})` : `LFS Files (${status.files.length})`}
                </button>
              ))}
            </div>
                ...
                <div style={{ maxHeight: 200, overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
                  {status.tracked.length === 0 ? (
                    ...
                    status.tracked.map((t) => (
                      ...
                      >
                        <span className="mono" style={{ flex: 1, color: "var(--text-primary)" }}>{t.pattern}</span>
