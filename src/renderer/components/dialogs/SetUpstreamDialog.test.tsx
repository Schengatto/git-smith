...
const mockElectronAPI = {
  remote: {
    list: vi
      .fn()
      .mockResolvedValue([{ name: "origin", fetchUrl: "https://github.com/x/y.git" }]),
...
describe("SetUpstreamDialog", () => {
  ...
  it("shows Push & Set Upstream confirm button", () => {
    ...
    expect(
      screen.getByRole("button", { name: /push & set upstream/i })
    ).toBeInTheDocument();
  });
  ...
  it("shows Force Push & Set Upstream label when force=true", () => {
    ...
    expect(
      screen.getByRole("button", { name: /force push & set upstream/i })
    ).toBeInTheDocument();
  });
