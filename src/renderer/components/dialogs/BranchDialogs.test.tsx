...
describe("DeleteBranchDialog", () => {
  ...
  it("shows force delete checkbox", () => {
    render(<DeleteBranchDialog open={true} onClose={vi.fn()} branchName="feature/x" />);
    expect(
      screen.getByText("Force delete (even if not fully merged)")
    ).toBeInTheDocument();
  });
});
...
describe("RebaseBranchDialog", () => {
  it("renders nothing when closed", () => {
    const { container } = render(
      <RebaseBranchDialog open={false} onClose={vi.fn()} onto="main" />
    );
    expect(container.innerHTML).toBe("");
  });
  ...
  it("renders when open", () => {
    const { container } = render(
      <RebaseBranchDialog open={true} onClose={vi.fn()} onto="main" />
    );
    expect(container.innerHTML).not.toBe("");
  });
