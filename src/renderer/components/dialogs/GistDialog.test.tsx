...
describe("GistDialog", () => {
  ...
  it("renders filename, description and content fields", () => {
    ...
    expect(screen.getByPlaceholderText("Paste or type your snippet here...")).toBeInTheDocument();
  });
  ...
  it("prefills initialContent and initialFilename", () => {
    ...
    expect(
      (screen.getByPlaceholderText("Paste or type your snippet here...") as HTMLTextAreaElement)
        .value
    ).toBe("hello world");
  });
  ...
  it("switching to Public sets public: true in create options", () => {
    render(
      <GistDialog open={true} onClose={vi.fn()} initialContent="code" initialFilename="f.ts" />
    );
  ...
  it("shows success state with gist URL after creation", async () => {
    render(
      <GistDialog open={true} onClose={vi.fn()} initialContent="hello" initialFilename="f.txt" />
    );
    fireEvent.click(screen.getByText("Create Gist"));
    await waitFor(() => {
  ...
  it("copies URL to clipboard when copy button is clicked after creation", async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith("https://gist.github.com/abc123");
  });
  ...
  it("resets state when dialog re-opens", async () => {
    ...
    rerender(
      <GistDialog open={false} onClose={vi.fn()} initialContent="code" initialFilename="f.ts" />
    );
    rerender(
      <GistDialog open={true} onClose={vi.fn()} initialContent="code" initialFilename="f.ts" />
    );
