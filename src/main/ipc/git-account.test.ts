...
vi.mock("../store", () => ({
  ...
  setDefaultAccountId: vi.fn((id: string | null) => {
    mockDefaultAccountId = id;
  }),
}));
...
describe("git-account IPC handlers", () => {
  ...
  it("SET_FOR_REPO applies account when repo is open", async () => {
    ...
    expect(gitService.applyAccount).toHaveBeenCalledWith("John Doe", "john@personal.com", {
      signingKey: "",
      sshKeyPath: "/home/john/.ssh/id_ed25519",
    });
  });
  ...
  it("SET_DEFAULT applies account globally", async () => {
    ...
    expect(gitService.applyAccount).toHaveBeenCalledWith("John Work", "john@work.com", {
      signingKey: "",
      sshKeyPath: "",
      global: true,
    });
...
describe("parseSshConfig", () => {
  ...
  it("parses host entries with identity files", () => {
    ...
    expect(entries[0]!.identityFile?.replace(/\\/g, "/")).toBe(
      path.join(tmpDir, ".ssh", "id_work").replace(/\\/g, "/")
    );
