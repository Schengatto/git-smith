...
export function registerStatsAdvancedHandlers() {
  ipcMain.handle(IPC.STATS_ADVANCED.TIMELINE, async (_event, period?: "day" | "week" | "month") => {
    return gitService.getTimeline(period);
  });

  ipcMain.handle(IPC.STATS_ADVANCED.CHURN, async (_event, period?: "day" | "week" | "month") => {
