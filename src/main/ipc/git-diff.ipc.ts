...
export function registerDiffHandlers() {
  ipcMain.handle(IPC.DIFF.FILE, async (_event, file: string, staged?: boolean) => {
  ...
  ipcMain.handle(IPC.DIFF.COMMIT_FILE, async (_event, hash: string, file: string) => {
  ...
  ipcMain.handle(IPC.DIFF.RANGE_FILES, async (_event, hash1: string, hash2: string) => {
