...
export function registerDialogHandlers(): void {
  ipcMain.handle(IPC.DIALOG.OPEN, (_event: IpcMainInvokeEvent, request: DialogOpenRequest) => {
    const win = windowManager.openDialog(request.dialog, request.data);
    return { windowId: win.id };
  });

  ipcMain.handle(IPC.DIALOG.CLOSE, (_event: IpcMainInvokeEvent, dialogKey: string) => {
