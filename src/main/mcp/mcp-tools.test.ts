...
function createMockGit() {
  return {
    ...
    getCommitDetails: vi
      .fn()
      .mockResolvedValue({ hash: "abc123", subject: "test commit" }),
    ...
    getCommitFiles: vi
      .fn()
      .mockResolvedValue([{ path: "src/a.ts", additions: 5, deletions: 2 }]),
...
describe("registerMcpTools", () => {
  ...
  describe("tool registration", () => {
    ...
    it("registers all read-only tools", () => {
      for (const name of allReadOnlyTools) {
        expect(server.registeredTools.has(name), `Missing read-only tool: ${name}`).toBe(
          true
        );
      }
    });
    ...
    it("registers all mutating tools", () => {
      for (const name of allMutatingTools) {
        expect(server.registeredTools.has(name), `Missing mutating tool: ${name}`).toBe(
          true
        );
      }
    });
  ...
  describe("conflict_save_merged", () => {
    it("calls git.saveMergedFile with file and content", async () => {
      ...
      expect(mockGit.saveMergedFile).toHaveBeenCalledWith(
        "src/conflict.ts",
        "merged content here"
      );
      expect(result.content[0]!.text).toContain("src/conflict.ts");
