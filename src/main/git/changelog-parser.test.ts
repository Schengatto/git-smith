...
describe("parseChangelog", () => {
  it("groups feat commits under Features", () => {
    const entries: ChangelogEntry[] = [makeEntry({ subject: "feat: add changelog dialog" })];
  ...
  it("groups fix commits under Bug Fixes", () => {
    const entries: ChangelogEntry[] = [makeEntry({ subject: "fix: broken graph lines" })];
    const result = parseChangelog(entries, "v1.0.0", "abc1234");
  ...
  it("extracts scope from conventional commit", () => {
    const entries: ChangelogEntry[] = [makeEntry({ subject: "feat(auth): add login flow" })];
    const result = parseChangelog(entries, "v1.0.0", "abc1234");
    const feats = result.groups.find((g) => g.label === "Features");
  ...
  it("detects breaking change via ! suffix", () => {
    const entries: ChangelogEntry[] = [makeEntry({ subject: "feat!: remove legacy API" })];
    const result = parseChangelog(entries, "v1.0.0", "abc1234");
  ...
  it("puts non-conventional commits in Other group", () => {
    const entries: ChangelogEntry[] = [makeEntry({ subject: "update readme" })];
    const result = parseChangelog(entries, "v1.0.0", "abc1234");
  ...
  it("omits empty groups from output", () => {
    const entries: ChangelogEntry[] = [makeEntry({ subject: "feat: only feature" })];
    const result = parseChangelog(entries, "v1.0.0", "abc1234");
